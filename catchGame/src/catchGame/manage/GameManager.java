...
public class GameManager {
	...
	public GameManager() {
		user = new User();
		this.isRunning = true;
		this.newMap = new MapExploring();
	}

	public void selectMap() throws InterruptedException {
//		this.newMap = new MapExploring();
		newMap.mapInput();
//		System.out.println(newMap.mapData);
//		this.location = newMap.mapData.get(0); // 첫 번째 선택한 맵으로 위치 설정
//		this.mapCount = newMap.mapData.size();
        this.location = newMap.mapData.get(newMap.mapData.size() - 1); // 마지막 선택한 맵으로 위치 설정
        System.out.println(newMap.mapData);
        System.out.println("\n-- " + this.location + " 맵으로 이동합니다 --");
//		System.out.println("\n-- " + this.location + " 맵으로 이동합니다 --");
	}
	...
	public void printGameMenu() throws InterruptedException {
		...
		switch (inputChoice) {
			case "1":
				...
				System.out.println();
				Thread.sleep(500);
				System.out.println("--" + this.location + "맵에 소환되었습니다" + "--");
				Thread.sleep(500);
				...
				user.catchMonster(this.location);
				printUserAction();
				break;
			...
			case "4":
				System.out.println("\n>> 내 정보를 확인합니다.");
				user.printUserInfo(this.location);
				break;
			case "5":
				System.out.println("\n>> 게임을 종료합니다. 감사합니다!");
